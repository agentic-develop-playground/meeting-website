...
const mobileRoutes = ['my-personal'];
watch(
  () => isPhone.value,
  (val) => {
    if (!val && mobileRoutes.includes(route.name)) {
      jumpToPage(props.items?.[0]?.path, true);
    }
  }
);
</script>
...
<template>
  <ContentCard class="layout-aside">
    <div class="user-info">
      <AppAvatar :avatar="userInfoStore.photo" :name="userInfoStore.username" :custom-size="64" />
