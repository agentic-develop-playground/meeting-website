...
interface RequestConfig<D = any> extends AxiosRequestConfig {
  ...
  skipAuth?: boolean; // 响应状态码401时是否跳过，默认为false
...
const responseInterceptorId = request.interceptors.response.use(
  (response: AxiosResponse) => {
    ...
    if ([undefined, 200, '200'].every((code) => code !== response?.data?.code) && config?.showError !== false) {
      const msg = useMessage();
      if (config.handleError) {
        config.handleError(response);
      }
      msg.show({
        content: i18n.global.t(`response.${response?.data?.msg}`, response?.data?.msg),
        status: 'danger',
      });
  ...
  (err: AxiosError) => {
    ...
    if (err.response) {
      if (config.handleError) {
        config.handleError(err.response);
      }
    ...
    if (config && config.showError !== false && config.ignoreError !== err.response?.status) {
      ...
      if (isObject(msg)) {
        ...
        msg = i18n.global.locale.value === 'en' ? message_en : message_zh;
      }

      message.show({
        content: i18n.global.t(`response.${msg || err.message}`, msg || err.message),
        status: 'danger',
      });
    }
